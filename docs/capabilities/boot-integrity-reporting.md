# `boot-integrity-reporting`: the published record must state the provenance of the key database and whether it can be changed at runtime, and the root-of-trust indicator must come from a defined reader that can only report an authenticated root on positive evidence - it is currently a constant.
