...
export AVOCADO_SDK_PREFIX="__SDK_INSTALL_PATH__"
export AVOCADO_SDK_SYSROOTS="${AVOCADO_SDK_PREFIX}/sysroots"
export DNF_SDK_HOST_PREFIX="${AVOCADO_SDK_PREFIX}"
export AVOCADO_SDK_MACHINE_TYPE="${AVOCADO_SDK_MACHINE_TYPE}"
...
if [ -f "${AVOCADO_SDK_PREFIX}/environment-setup" ]; then
    echo "--- Entrypoint: Found ${AVOCADO_SDK_PREFIX}/environment-setup. Sourcing it and skipping SDK installation steps. ---"
...
echo "--- Entrypoint: Changing working directory to /opt ---"
cd /opt
