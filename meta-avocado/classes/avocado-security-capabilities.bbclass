...
# can disagree, and only the recipe-side check in optee-ftpm-init catches it,
# and only in one direction. Upgrade trigger: ftpm.yml's MACHINE_FEATURES
# append is replaced with a read of the selected machine's
# AVOCADO_SECURITY_CAPABILITIES declaration.
