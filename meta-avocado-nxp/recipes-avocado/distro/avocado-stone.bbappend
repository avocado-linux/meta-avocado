...
do_deploy:append() {
  ...
  install -m 0644 ${WORKDIR}/imx/rootdisk.conf ${DEPLOYDIR}/stone/rootdisk.conf
}
...
do_stone_provision:append() {
  ...
  stone_manifest="${DEPLOY_DIR_IMAGE}/stone/stone-${MACHINE_SHORT_NAME}.json"
  ...
  fwup -c -f "${DEPLOY_DIR_IMAGE}/stone/rootdisk.conf" -o "${DEPLOY_DIR_IMAGE}/stone/${MACHINE_SHORT_NAME}-rootdisk.zip"
  fwup -a -d "${DEPLOY_DIR_IMAGE}/stone/${MACHINE_SHORT_NAME}-rootdisk.img" -i "${DEPLOY_DIR_IMAGE}/stone/${MACHINE_SHORT_NAME}-rootdisk.zip" -t complete
}
