...
# Same rationale: the `kernel-modules` meta-package emitted by kernel.bbclass
# ships under its bare name with no KERNEL_VERSION qualifier, so a multi-kernel
# feed gets two RPMs colliding on the same NAME and dnf NVR-tiebreaks. Renaming
# it lets BSP yamls list `kernel-modules` and have avocado-cli's auto-suffix
# resolve to this kernel's meta — pulling in every per-module RPM transitively.
