...
# (the partition math uses 2048 sectors per MiB), so BLOCK_SIZE is stone
# metadata only. Parsed here to keep this script aligned with its stm sibling.
...
size_to_mib() {
  ...
  case "$unit" in
    ...
    *)
      echo "ERROR: unknown size unit: $unit" >&2
      exit 1
      ;;
  esac
}
