...
DEPENDS += "wlan-platform qcacld-wlan coreutils-native"
...
do_compile[depends] += "wlan-platform:do_populate_sysroot"

python __anonymous () {
    d.setVar('KBUILD_EXTRA_SYMBOLS', d.getVar('STAGING_DIR_HOST') + "/usr/include"  + "/wlan-platform/Module.symvers" )
    machine = d.getVar('MACHINE')
    if ("qcs6490" in machine) or ("qcm6490" in machine) or ("rubikpi3" in machine) or ("avocado-rubikpi3" in machine):
