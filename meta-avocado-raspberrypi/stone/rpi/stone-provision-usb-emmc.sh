...
# RPi eMMC provisioning via rpiboot USB mass storage.
#
# This script provisions an eMMC-equipped RPi (CM4/CM5) by:
# 1. Building a raw disk image from the firmware archive
# 2. Patching the U-Boot environment for the eMMC target device
# 3. Using rpiboot to expose the eMMC as USB mass storage
# 4. Writing the patched image to the eMMC
...
echo "=== RPi eMMC Provisioning via rpiboot ==="
...
# --- Step 2: Patch U-Boot env for eMMC target ---
echo "Patching U-Boot environment for eMMC..."
...
# Extract env files from the FAT partition inside the disk image using mtools,
# patch them with fw_setenv, then write them back.
# mtools accesses FAT at a byte offset with: mcopy -i image@@offset
...
# Point fw_setenv at the extracted env files
...
# On RPi, the boot device is always mmcblk0 regardless of SD or eMMC.
fw_setenv -c "$fw_env_config" mmcblk 0
echo "U-Boot env patched for eMMC target"

# Write patched env files back into the disk image FAT partition
...
# Write the .img (which has the patched env) directly to the device.
# We can't use fwup here because it would re-apply the original archive
# and overwrite our env patches.
echo "Writing patched system image to eMMC..."
...
if ! dd if="${archive_image}" of="${rpi_block_device}" bs=4M status=progress conv=fsync; then
    echo "Error: Failed to write system image to eMMC"
    exit 1
fi
...
echo "Ensure the EEPROM boot order includes eMMC and remove any SD card."
