...
# 3. Patching the U-Boot environment with device identity
# 4. Patching cmdline.txt for NVMe root device
# 5. Using rpiboot to expose the NVMe as USB mass storage
# 6. Writing the image to the NVMe device
#
# With tryboot, the EEPROM boots the kernel directly from NVMe —
# no U-Boot binary swap needed (unlike the old U-Boot flow).
...
echo "=== RPi NVMe Provisioning via rpiboot (tryboot) ==="
...
# --- Step 2: Patch U-Boot env for device identity ---
echo "Patching U-Boot environment..."
...
# No cmdline patching needed — GPT PARTLABELs (root=PARTLABEL=rootfs-a)
# work on any storage device.
rpiboot_detect_and_expose

# --- Step 5: Write the image ---
echo "Writing system image to NVMe..."
