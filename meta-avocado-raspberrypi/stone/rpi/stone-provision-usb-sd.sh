...
# 2. Patching the U-Boot environment with device-specific values
# 3. Using rpiboot to expose the SD card as USB mass storage
# 4. Writing the patched image to the SD card
#
# The default U-Boot binary (CONFIG_ENV_FAT_INTERFACE="mmc", device 0:2)
# is correct for SD — no binary swap is needed.
...
echo "=== RPi SD Card Provisioning via rpiboot ==="
...
# --- Step 2: Patch U-Boot env for SD target ---
...
# SD card is always mmc 0 on RPi.
fw_setenv -c "$fw_env_config" mmcblk 0
echo "U-Boot env patched for SD target"
...
echo "Writing patched system image to SD card..."
